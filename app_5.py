...
def main():
    ...
    st.markdown("<h3>• RESERVAS Y RECURSOS DE HIDROCARBUROS (Año 2023)</h3>", unsafe_allow_html=True)
    for h in hidrocarburos:
        mostrar_reservas_recursos(df, lote, h)

    st.markdown("<h3>• PRODUCCIÓN FISCALIZADA DE HIDROCARBUROS</h3>", unsafe_allow_html=True)
    for h in hidrocarburos:
        mostrar_produccion(df, lote, h)

    df_inv = mostrar_inversion(df, lote)
    if df_inv is not None:
        st.markdown("<h3>• INVERSIÓN (MMUSD)</h3>", unsafe_allow_html=True)
        st.table(df_inv)

    df_reg = mostrar_regalias(df, lote)
    if df_reg is not None:
        st.markdown("<h3>• REGALÍA (MMUSD)</h3>", unsafe_allow_html=True)
        st.table(df_reg)

    df_can = mostrar_canon(df, lote)
    if df_can is not None:
        st.markdown("<h3>• CANON (MMUSD)</h3>", unsafe_allow_html=True)
        st.table(df_can)
